class GuestReviewsController < ApplicationController
    ...
    def create
        # Step 1:Check if the reservation exist (room_id, host_id, guest_id)

        # Step 2:Check if the current host already reviewd the guest in this reservation.
        ...
        if !@reservation.nil? && @reservation.room.user.id == guest_review_params[:host_id].to_i
            ...
            if @has_reviewd.nil?
                # Allow to review
                @guest_reviewd = current_user.guest_reviews.create(guest_review_params)
    ...
    def destroy
        ...
        redirect_back(fallback_location: request.referer, notice: "Remove...!")
    end
